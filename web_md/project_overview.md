...
|---|---|---|
| `MenuAppController` | Điều khiển scaffold key (mở/đóng drawer) | `controllers/menu_app_controller.dart` |
| `DashboardController` | Fetch và giữ data cho dashboard tổng quan | `controllers/dashboard_controller.dart` |
| `AuthController` | Quản lý trạng thái đăng nhập (dùng SharedPreferences) | `controllers/auth_controller.dart` |
...
## 6. Navigation — Router tĩnh (MaterialApp)
- Hiện tại dùng `home: MainScreen()` (không có named routes).
- **Kế hoạch:** Chuyển sang `MaterialApp.router` + `go_router` khi số màn hình > 5.
- Mỗi mục trong SideMenu sẽ thay đổi nội dung vùng content bằng cách update state trong `MenuAppController`.
